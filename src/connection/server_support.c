...
css_process_get_server_ha_mode_request (SOCKET master_fd)
{
  ...
  if (HA_DISABLED ())
    {
      response = htonl (ha_operations::SERVER_STATE_NA);
    }
  else
    {
...
css_process_change_server_ha_mode_request (SOCKET master_fd)
{
  ...
  if (state == ha_operations::SERVER_STATE_ACTIVE || state == ha_operations::SERVER_STATE_STANDBY)
    {
      if (ha_operations::change_server_state (thread_p, state, false, HA_CHANGE_MODE_IMMEDIATELY, true) != NO_ERROR)
	{
...
css_process_master_hostname ()
{
  ...
  assert (hostname_length > 0 && (ha_operations::get_server_state () == ha_operations::SERVER_STATE_TO_BE_STANDBY
          || ha_operations::get_server_state () == ha_operations::SERVER_STATE_STANDBY));
  ...
  {
    cubreplication::replication_node_manager::wait_commute (ha_operations::get_server_state (), ha_operations::SERVER_STATE_STANDBY);
...
css_connection_handler_thread (THREAD_ENTRY * thread_p, CSS_CONN_ENTRY * conn)
{
  ...
  while (thread_p->shutdown == false && conn->stop_talk == false)
      ...
      if (n == 0)
	{
	  ...
	  if (ha_operations::get_server_state () == ha_operations::SERVER_STATE_TO_BE_STANDBY
	  ...
	  if (logtb_count_clients (thread_p) == 1)
	    {
	      ...
	      state = ha_operations::transit_server_state (thread_p, ha_operations::SERVER_STATE_STANDBY);
	      assert (state == ha_operations::SERVER_STATE_STANDBY);
	      if (state == ha_operations::SERVER_STATE_STANDBY)
		{
...
css_check_ha_log_applier_working (void)
{
  ...
  if (i == ha_Server_num_of_hosts
      && (ha_operations::get_server_state () == ha_operations::SERVER_STATE_TO_BE_STANDBY
	  || ha_operations::get_server_state () == ha_operations::SERVER_STATE_STANDBY))
    {
      return true;
    }
  ...
  if (css_check_ha_log_applier_done ())
    {
      ...
      server_state = ha_operations::transit_server_state (thread_p, ha_operations::SERVER_STATE_ACTIVE);
      assert (server_state == ha_operations::SERVER_STATE_ACTIVE);
      if (server_state == ha_operations::SERVER_STATE_ACTIVE)
	{
  ...
  if (css_check_ha_log_applier_working ())
    {
      ...
      server_state = ha_operations::transit_server_state (thread_p, ha_operations::SERVER_STATE_STANDBY);
      assert (server_state == ha_operations::SERVER_STATE_STANDBY);
      if (server_state == ha_operations::SERVER_STATE_STANDBY)
	{
...
css_process_new_slave (SOCKET master_fd)
{
  ...
  // *INDENT-OFF*
  cubreplication::replication_node_manager::inc_ha_tasks ();
  assert (ha_operations::get_server_state () == ha_operations::SERVER_STATE_TO_BE_ACTIVE || ha_operations::get_server_state () == ha_operations::SERVER_STATE_ACTIVE);
  cubthread::entry_task * new_slave_task = new cubthread::entry_callable_task ([new_fd] (cubthread::entry & context)
  {
    cubreplication::replication_node_manager::wait_commute (ha_operations::get_server_state (), ha_operations::SERVER_STATE_ACTIVE);
